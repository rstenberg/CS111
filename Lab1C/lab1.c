...
#include <sys/resource.h> // profile
	...
	while((c = getopt_long(argc, argv, "", long_options, &option_index)) != -1)	{
		...
		switch(c)	{
			...
			case 'r':		// Handle "--rdonly f" argument
				...
				int rd_fd = open(optarg, read_file_flags, 0644);
				if (rd_fd < 0) {
			...
			case 'w':		// Handle "--wronly f" argument
				...
				int wr_fd = open(optarg, write_file_flags, 0644);
				if (wr_fd < 0) {
			...
			case '&':		// Handle "--rdwr f" argument
				...
				int rdwr_fd = open(optarg, rdwr_file_flags, 0644);
				if (rdwr_fd < 0) {
			...
			case 'c':		// Handle "--command i o cmd args" option/args
				...
				for (int i = 1; i <= arg_cnt; i++)	{
					...
					full_command = (char*)realloc(full_command, (strlen(full_command) + arg_len + 1) * sizeof(char));
					full_command = strcat(full_command, " ");
			...
			case '1':	// Handle "--abort" 
				...
				// Print abort option if verbose flag set
				if (vb_flag)	{
					printf("--abort\n");
				}
			...
			case '2':	// Handle "--catch N"
				...
				// Print catch N option if verbose flag set
				if (vb_flag)	{
					printf("--catch %s\n", argv[optind - 1]);
				}
			...
			case '3':	// Handle "--ignore N"
				...
				// Print ignore N option if verbose flag set
				if (vb_flag)	{
					printf("--ignore %s\n", argv[optind - 1]);
				}
			...
			case '4':	// Handle "--default N"
				...
				// Print default N option if verbose flag set
				if (vb_flag)	{
					printf("--default %s\n", argv[optind - 1]);
				}
			...
			case '5':	// Handle "--pause"
				...
				// Print pause option if verbose flag set
				if (vb_flag)	{
					printf("--pause\n");
				}
